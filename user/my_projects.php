...
<body >
		...
		<div class = "logintext">
			<h2 style = "color:#3871F6; width: auto;"><b> My Projects</b></h2>
				...
				<div class = "col " style = "text-align:center" >
					...
					<?php			
						...
						if(isset($_POST['search']))
						{
							...
							{
								$qry = "SELECT * FROM `project_information` where `semester` = '$semester' AND `supervisor` ='$_SESSION[name]'";
							}
							else if($semester == "" && $search_name_wise == "")
							{
								$qry = "SELECT * FROM `project_information` where `session` = '$session' AND `supervisor` ='$_SESSION[name]'";
							}	
							else if ($session == "" && $semester == "")
							{
								$qry = "select * from `project_information` where `project_name` like '%$_POST[search_namewise]%' AND `supervisor` ='$_SESSION[name]'";
							}
							else if($search_name_wise == "")
							{
								$qry = "SELECT * FROM `project_information` where `session` = '$session' AND `semester` = '$semester' AND `supervisor` ='$_SESSION[name]'";
							}
							else if($session =="")
							{
								$qry = "select * from `project_information` where `project_name` like '%$_POST[search_namewise]%' AND `semester` = '$semester' AND `supervisor` ='$_SESSION[name]'";
							}
							else if($semester == "")
							{
								$qry = "select * from `project_information` where `project_name` like '%$_POST[search_namewise]%' AND `session` = '$session' AND `supervisor` ='$_SESSION[name]'";
							}
							else
							{
								$qry = "select * from `project_information` where `project_name` like '%$_POST[search_namewise]%' AND `session` = '$session' AND `semester` = '$semester' AND `supervisor` ='$_SESSION[name]'";
							}
						...
						else
						{
							...
							$start = ($page-1) * $limit; 
							$qry = "select * from `project_information` where `supervisor` ='$_SESSION[name]' LIMIT $start, $limit ";
							$res = mysqli_query($conn, $qry);
							
							$countqry = "select count(id) as id from `project_information` where `supervisor` ='$_SESSION[name]'";
										...
										<ul class="pagination pagination-lg">
											<li class="page-item">
												<?php
													if($page>1)
													{
														echo "<a class='inactive_page page-link' href='my_projects.php?page=".$previous."'> Previous</a> ";
													}
													else
													{
											...
											<?php 
												...
												if($page==$i)
												{
													echo "<a class='active_page page-link'  href='my_projects.php?page=".$i."'>".$i."</a> ";
												}
												else
												{
													echo "<a class=' inactive_page page-link'  href='my_projects.php?page=".$i."'>".$i."</a> ";
												}						
												}; 
											?>
											...
											<li class="page-item">
												<?php 
													if($pages>=$next)
													{
														echo "<a class='inactive_page page-link' href='my_projects.php?page=".$next."'> Next</a> ";	
													}
													else
													{
