				...
				<div class = "col " style = "text-align:center" >
					...
					<?php
						...
						if(isset($_POST['search']))
						{
							...
							{
								$qry = "select * from `project_information` where `project_name` like '%$_POST[search_namewise]%' AND `semester` = '$semester'";
							}
							else if($semester == "")
							{
								$qry = "select * from `project_information` where `project_name` like '%$_POST[search_namewise]%' AND `session` = '$session'";
							}
							else
							{
								$qry = "select * from `project_information` where `project_name` like '%$_POST[search_namewise]%' AND `session` = '$session' AND `semester` = '$semester'";
							}
							...
							echo "<table class='table table-bordered table-dark'>";	
									...
									while($row = mysqli_fetch_assoc($res))
									{
										...
										echo "<tr>";
											...
											echo "<td>";
											?>
											 <a class = "btn btn-danger" style = "text-decoration:none; color: white;" href="view_project_details_user.php?id=<?php echo $row['id']; ?>">View Details</a> 
											<?php	
											echo "</td>";													
										echo "</tr>";
									}
										...
										<ul class="pagination pagination-lg">
											<li class="page-item">
												<?php
													if($page>1)
													{
														echo "<a class='inactive_page page-link' href='user_panel.php?page=".$previous."'> Previous</a> ";
													}
													else
													{
											...
											<?php 
												...
												if($page==$i)
												{
													echo "<a class='active_page page-link'  href='user_panel.php?page=".$i."'>".$i."</a> ";
												}
												else
												{
													echo "<a class=' inactive_page page-link'  href='user_panel.php?page=".$i."'>".$i."</a> ";
												}						
												}; 
											?>
											...
											<li class="page-item">
												<?php 
													if($pages>=$next)
													{
														echo "<a class='inactive_page page-link' href='user_panel.php?page=".$next."'> Next</a> ";	
													}
													else
													{
