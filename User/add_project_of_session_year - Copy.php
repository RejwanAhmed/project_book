		...
		<?php 
			...
			if(isset($_POST['submit1']))
			{
				...
				if(empty($_FILES['upload_image']['name']))
				{
					...
					else
					{
						...
						<script>
							...
							window.location = "add_project_of_session_year.php?id=$_GET['id']";
						</script>
						<?php	
					}
				...
				{	
					...
					{
						move_uploaded_file($_FILES['file']['tmp_name'],'pdf/'.$_FILES['file']['name']);
						$qry3 = "INSERT INTO `project_information`( `project_name`, `session`, `year`, `semester`, `supervisor`, `co_supervisor`, `member1`, `member2`, `member3`, `project_des`, `project_image`,`pdf`) VALUES ('$project_name','$session','$year','$semester','$supervisor','$co_supervisor','$member1', '$member2','$member3','$project_description','.$upload_file','$resizeFileName.$fileExt')";
